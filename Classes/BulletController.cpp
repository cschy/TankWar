...
{
	//do some action
	AudioEngine::play2d("sounds/collide_others.mp3");
	tankFlash();
...
void BulletController::tankBoom()
{
	...
	m_hitTank->runAction(Sequence::create(action, CCRemoveSelf::create(), CallFunc::create([&]() {m_hitTank->removeFromParentAndCleanup(true); }), NULL));
}
